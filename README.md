...
- **历史资源查询**：按站点和每批虚拟机数量批量查询 CPU、内存、磁盘历史曲线；前端可选 10、20、30、50 或 100 台/批。
...
- 页面默认查询最近 7 天，可自行设置开始和结束时间。
- 结束时间必须晚于开始时间；单次查询范围最长为 **366 天**。
- 页面可选粒度为 5 分钟、30 分钟、1 小时、1 天。粒度越细，采样点与 VRM 响应量通常越多。
...
可选 10、20、30、50、100 台/批，默认 50 台。程序还会按站点边界分批，避免将不同站点的虚拟机放入同一个历史指标请求。
