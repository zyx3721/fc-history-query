...
- **资源汇总**：输出每个指标的最大值、平均值和低于阈值的采样点比例；同时展示虚拟机在当前查询范围内的最早有效采集时间，缺失历史数据会显式标记为“无数据”。
...
采样点是**某一台虚拟机、某一个资源指标在查询时间范围内的一个时间点数据**，不是虚拟机数量。查询结果中的“最早有效采集时间”取所选指标返回的有效采样点中最早的时间，并按浏览器本地时间显示；它不等同于虚拟机创建或开机时间。
...
服务会在终端以单行键值对输出查询相关操作日志，包含 `time`（操作时间）、`client_ip`（发起请求的客户端 IP）、`operation`（操作类型）和任务 ID 等上下文。例如：

```text
time="2026-07-21T10:30:00.123456789+08:00" client_ip="192.0.2.10" operation="query.completed" job_id="..." scanned="100" matched="24" duration="2.75s"
```

每个查询任务会记录一条 `query.created` 创建日志，并在完成、失败或最终取消时记录一条结果日志；页面自动轮询状态不会写日志。取消请求和参数校验拒绝仍会单独记录。为避免泄露敏感信息，日志不会记录用户名、密码、`X-Auth-Token`、完整请求体或 VRM 返回错误正文。`client_ip` 来自与服务直接建立 TCP 连接的地址；部署在反向代理后时，该地址通常是代理服务器 IP。
...
## 10.1 v1.0.2 - 2026-07-21

新增查询操作终端日志，记录操作时间、客户端 IP、任务 ID 和查询结果统计；自动状态轮询不再重复写入终端日志。

详细更新内容见 [verchanglog/v1.0.2.md](verchanglog/v1.0.2.md)。

## 10.2 v1.0.1 - 2026-07-19

新增 1 分钟、10 分钟、30 天时间粒度，默认查询最近 30 天；查询结果新增最早有效采集时间展示、排序、搜索和 Excel 导出支持。

详细更新内容见 [verchanglog/v1.0.1.md](verchanglog/v1.0.1.md)。

## 10.3 v1.0.0 - 2026-07-18
